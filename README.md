...
**뷰티샵 종합 관리 Electron 앱**입니다.

Turborepo 모노레포로 구축하여 고객·예약·직원·서비스 관리 기능을 제공합니다.
재사용 가능한 컴포넌트 시스템과 증분 빌드로 확장성과 개발 효율을 높였습니다.
...
BeautyManager는 뷰티샵 운영에 필요한 고객·예약·직원·서비스를 통합 관리하는
크로스 플랫폼 데스크톱 애플리케이션입니다. Turborepo 모노레포 구조로 공통
컴포넌트를 재사용하며, SQLite 로컬 데이터베이스로 오프라인에서도 안정적으로
동작합니다.

**현재 구현:**

- Electron 기반 데스크톱 앱 (Windows/macOS/Linux)
- SQLite 로컬 데이터베이스
- 공유 컴포넌트 라이브러리 (@nexus/ui)

**향후 계획:**

- 웹 애플리케이션 배포 (React SPA)
- RESTful API 백엔드 서버 개발 (NestJS)
- 클라우드 데이터베이스 연동 (PostgreSQL)
- 데스크톱/웹 간 데이터 동기화

**프로젝트 목표:**

- Electron으로 Windows/macOS/Linux 모두 지원하는 데스크톱 앱 개발
- 모노레포 구조로 컴포넌트 재사용성과 유지보수성 확보
- Turborepo 증분 빌드와 원격 캐싱으로 빌드 시간 70% 단축
- TypeScript strict 모드와 타입 안전 상태 관리로 런타임 에러 최소화
- shadcn/ui 기반 디자인 시스템으로 일관된 사용자 경험 제공
