...
export default function StaffDetailModal({
  ...
  const [activeTab, setActiveTab] = useState<
    'info' | 'performance' | 'customers' | 'holidays'
  >('info');
  ...
  const handlePositionChange = (newPosition: Staff['position']) => {
    if (
      confirm(
        `정말 ${staff.name}의 직급을 ${getPositionLabel(staff.position)}에서 ${getPositionLabel(newPosition)}으로 변경하시겠습니까?`
      )
    ) {
  ...
  const monthlyData = [
    ...
    {
      month: '6월',
      customers: currentStaff.monthlyCustomers,
      services: currentStaff.monthlyServices,
    },
  ];
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <span
                      className={`inline-block rounded-full px-2 py-1 text-xs font-medium ${getRoleBadgeColor(currentStaff.role)}`}
                    >
                      {currentStaff.role}
                    </span>
                    <span
                      className={`inline-block rounded-full px-2 py-1 text-xs font-medium ${getPositionBadgeColor(currentStaff.position)}`}
                    >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-900">
                          {new Date(currentStaff.hireDate).toLocaleDateString(
                            'ko-KR'
                          )}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-900">
                          {calculateTenure(currentStaff.hireDate)}
                        </span>
                      </div>
                    </div>
                  </Card>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                  <Card>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-blue-600">
                        {
                          currentStaff.holidays.filter(
                            (h) =>
                              new Date(h.startDate).getMonth() ===
                                new Date().getMonth() &&
                  ...
                  <Card>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-red-600">
                        {
                          currentStaff.holidays.filter((h) => h.type === 'sick')
                            .length
                        }
                      </p>
                ...
                <Card>
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    <Button
                      ...
                      className="text-sm"
                    >
                      <i className="ri-add-line mr-2" />새 휴일 등록
                    </Button>
                  </div>
                    ...
                    <div className="space-y-4 border-t border-gray-200 pt-4">
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div className="flex items-center">
                          <label className="flex cursor-pointer items-center">
                            ...
                            />
                            <div
                              className={`relative h-5 w-5 rounded border-2 ${
                            ...
                            >
                              {newHoliday.isHalfDay && (
                                <i className="ri-check-line absolute inset-0 flex items-center justify-center text-sm text-white" />
                              )}
                            </div>
                      ...
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <Button
                          ...
                          disabled={
                            !newHoliday.startDate || !newHoliday.reason.trim()
                          }
                          className="text-sm"
                        >
                          등록
                  ...
                  <div className="space-y-3">
                    {currentStaff.holidays.length === 0 ? (
                      <div className="py-8 text-center">
                        <i className="ri-calendar-line mb-2 text-4xl text-gray-300" />
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div className="mb-1 flex items-center gap-2">
                                <span
                                  className={`rounded-full px-2 py-1 text-xs font-medium ${getHolidayTypeColor(holiday.type)}`}
                                >
                                  {getHolidayTypeLabel(holiday.type)}
                                </span>
                                <span
                                  className={`rounded-full px-2 py-1 text-xs font-medium ${getStatusColor(holiday.status)}`}
                                >
                              ...
                              <p className="mt-1 text-xs text-gray-500">
                                신청일:{' '}
                                {new Date(holiday.appliedAt).toLocaleDateString(
                                  'ko-KR'
                                )}
                              </p>
                            </div>
                          ...
                          <div className="flex items-center gap-2">
                            {holiday.status === 'pending' && (
                              <>
                                <button
                                  onClick={() =>
                                    handleHolidayAction(holiday.id, 'approve')
                                  }
                                ...
                                <button
                                  onClick={() =>
                                    handleHolidayAction(holiday.id, 'reject')
                                  }
                            ...
                            <button
                              onClick={() =>
                                handleHolidayAction(holiday.id, 'delete')
                              }
