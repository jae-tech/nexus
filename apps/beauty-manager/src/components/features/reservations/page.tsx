...
import { format, startOfDay, endOfDay, startOfWeek, endOfWeek, startOfMonth, endOfMonth, subWeeks, subMonths, isWithinInterval } from 'date-fns';
...
export function Reservations() {
  ...
  const filteredReservations = useMemo(() => {
    ...
    filtered = filtered.filter(reservation => {
      ...
      return isWithinInterval(reservationDate, { start: dateRange.start, end: dateRange.end });
    });
    
    // 직원 필터
    if (selectedEmployee !== 'all') {
      filtered = filtered.filter(reservation => reservation.employeeId === selectedEmployee);
    }
    ...
    if (serviceTypeFilter !== 'all') {
      filtered = filtered.filter(reservation => {
        return reservation.services.some(service => {
          if (serviceTypeFilter === 'hair') return service.name.includes('컷') || service.name.includes('염색') || service.name.includes('펌');
          if (serviceTypeFilter === 'nail') return service.name.includes('네일');
          if (serviceTypeFilter === 'care') return service.name.includes('케어') || service.name.includes('마사지');
          return true;
        });
      });
    }
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(reservation => 
        ...
        reservation.services.some(service => service.name.toLowerCase().includes(query)) ||
  ...
  const handleAddReservation = (selectedDateTime?: { date: string; time: string }) => {
  ...
  const handleBulkStatusChange = (reservationIds: string[], newStatus: Reservation['status']) => {
    setReservationList(prev => prev.map(r => 
  ...
  const handleBulkDelete = (reservationIds: string[]) => {
    if (confirm(`선택한 ${reservationIds.length}개의 예약을 삭제하시겠습니까?`)) {
      setReservationList(prev => prev.filter(r => !reservationIds.includes(r.id)));
    }
  };
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="text-xs sm:text-sm px-2 sm:px-3 py-1.5 sm:py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 flex items-center gap-2"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        <a href={`tel:${selectedReservation.customerPhone}`} className="text-blue-600 hover:text-blue-800">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="font-medium">{format(new Date(selectedReservation.date), 'yyyy년 M월 d일')}</div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                        selectedReservation.status === 'scheduled' ? 'bg-blue-100 text-blue-800' :
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="border-t pt-2 flex justify-between items-center font-semibold">
                      <div>총 금액</div>
                      <div>{selectedReservation.amount?.toLocaleString() || 0}원</div>
                    </div>
                  </div>
                </div>
