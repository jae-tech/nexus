...
export function Customers() {
  ...
  const filteredAndSortedCustomers = useMemo(() => {
    let filtered = customerList.filter(customer => {
      ...
      switch (selectedFilter) {
        ...
        case 'new':
          const daysSinceRegistered = Math.floor((today.getTime() - registeredDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const getVisitStatus = (lastVisit: string) => {
    ...
    if (diffDays <= 7) return { text: `${diffDays}일 전`, color: 'text-green-600' };
    if (diffDays <= 30) return { text: `${diffDays}일 전`, color: 'text-yellow-600' };
  ...
  const getCustomerTypeLabel = (customer: Customer) => {
    ...
    if (daysSinceRegistered <= 30) return { text: 'NEW', color: 'bg-green-100 text-green-800' };
    if (customer.visitCount >= 10) return { text: 'VIP', color: 'bg-purple-100 text-purple-800' };
    if (customer.visitCount >= 5) return { text: '단골', color: 'bg-blue-100 text-blue-800' };
    return null;
  };
