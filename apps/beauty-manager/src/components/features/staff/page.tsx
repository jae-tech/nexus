...
export function Staff() {
  ...
  const handleViewSchedule = (staff: StaffType, e: React.MouseEvent) => {
    e.stopPropagation();
    setSelectedStaffType(staff);
  ...
  const getPositionBadgeColor = (position: StaffType["position"]) => {
    switch (position) {
      case "owner":
  ...
  const getPositionLabel = (position: StaffType["position"]) => {
    switch (position) {
      case "owner":
        return "사장";
      case "manager":
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span
                          className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getRoleBadgeColor(staff.role)}`}
                        >
                          {staff.role}
                        </span>
                        <span
                          className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getPositionBadgeColor(staff.position)}`}
                        >
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredAndSortedStaff.map((staff) => (
                    <tr
                      key={staff.id}
                      className={`border-b border-gray-100 hover:bg-gray-50 cursor-pointer ${staff.status === "terminated" ? "opacity-60" : ""}`}
                      onClick={() => handleViewDetail(staff)}
                    >
