...
import ListView from '@/features/appointments/components/ListView';
import AddAppointmentModal from '@/features/appointments/components/AddAppointmentModal';
import EditAppointmentModal from '@/features/appointments/components/EditAppointmentModal';
import {
  format,
  startOfDay,
  endOfDay,
...
export default function AppointmentsPage() {
  ...
  const [serviceList] = useState<any[]>([]);
  ...
  const filteredAppointments = useMemo(() => {
    ...
    if (selectedEmployee !== 'all') {
      filtered = filtered.filter(
        (appointment) => appointment.employeeId === selectedEmployee
      );
    }
  ...
  const handleAddAppointment = (selectedDateTime?: {
    date: string;
    time: string;
  }) => {
  ...
  const handleDeleteAppointment = (appointmentId: string, reason?: string) => {
    if (confirm('정말로 이 예약을 삭제하시겠습니까?')) {
      setAppointmentList((prev) => prev.filter((r) => r.id !== appointmentId));
    }
  };
  ...
  const handleStatusChange = (
    ...
    setAppointmentList((prev) =>
      prev.map((r) =>
        r.id === appointmentId ? { ...r, status: newStatus } : r
      )
    );
  };
  ...
  const handleBulkStatusChange = (
    ...
    setAppointmentList((prev) =>
      prev.map((r) =>
        appointmentIds.includes(r.id) ? { ...r, status: newStatus } : r
      )
    );
  };
  ...
  const handleBulkDelete = (appointmentIds: string[]) => {
    if (
      confirm(`선택한 ${appointmentIds.length}개의 예약을 삭제하시겠습니까?`)
    ) {
      setAppointmentList((prev) =>
        prev.filter((r) => !appointmentIds.includes(r.id))
      );
    }
  };
      ...
      {selectedAppointment && (
        <EditAppointmentModal
          ...
          onSave={(updatedAppointment) => {
            setAppointmentList((prev) =>
              prev.map((r) =>
                r.id === updatedAppointment.id ? updatedAppointment : r
              )
            );
            setShowEditModal(false);
            setSelectedAppointment(null);
                ...
                <div className="rounded-lg bg-gray-50 p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between border-t pt-2 font-semibold">
                      <div>총 금액</div>
                      <div>
                        {selectedAppointment.amount?.toLocaleString() || 0}원
                      </div>
                    </div>
                  </div>
                </div>
