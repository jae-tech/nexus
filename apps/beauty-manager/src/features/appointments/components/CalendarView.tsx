...
import { ArrowLeft, ArrowRight, MessageCircle, Phone, Plus, Scissors, Trash2, User } from 'lucide-react';
import {
  format,
  startOfWeek,
  endOfWeek,
    ...
    return (
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
        ...
        <div className="grid grid-cols-7">
          {days.map((day, index) => {
            const dayAppointments = appointments.filter((res) =>
                  ...
                  >
                    {dayAppointments.map((appointment) => (
                      <div
                        key={appointment.id}
                        onClick={() => onAppointmentClick(appointment)}
                        className={`cursor-pointer rounded border-l-4 p-1 text-xs transition-opacity hover:opacity-80 md:p-2 ${getServiceCategoryColor(appointment.services[0]?.name || '')} ${getStatusColor(appointment.status)}`}
                      >
                        <div className="truncate font-medium">
                  ...
                  {slotAppointments.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {slotAppointments.map((appointment) => (
                        <div
                          key={appointment.id}
                          className={`cursor-pointer rounded-lg border-l-4 p-2 transition-shadow hover:shadow-md md:p-3 ${getServiceCategoryColor(appointment.services[0]?.name || '')} ${getStatusColor(appointment.status)}`}
                          onClick={() => onAppointmentClick(appointment)}
                        >
                          ...
                          <div className="mb-1 text-xs text-gray-600 md:text-sm">
                            <Phone size={18} className="mr-1 text-gray-600" />
                            {appointment.customerPhone}
                          </div>
                          <div className="mb-1 text-xs text-gray-600 md:text-sm">
                            <Scissors size={18} className="mr-1 text-gray-600" />
      ...
      <div className="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center md:mb-6">
        <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <ArrowLeft className="h-4 w-4 md:text-base" />
            </button>
            ...
            <button
              ...
              className="rounded-lg p-1.5 text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 md:p-2"
            >
              <ArrowRight className="h-4 w-4 md:text-base" />
            </button>
          </div>
        </div>
