...
export default function StaffPageDB() {
  ...
  // DB 훅 사용
  const { staff: dbStaff, loading: staffLoading, refetch: refetchStaff } = useStaff();
  const { staffByPosition: dbStaffByPosition } = useStaffByPosition();
  const { performance: staffPerformance } = useStaffPerformance();
  const { createStaff } = useCreateStaff();
  const { updateStaff } = useUpdateStaff();
  const { deleteStaff } = useDeleteStaff();

  // DB 직원을 프론트엔드 형식으로 변환
  const staffList = useMemo(() => {
    return dbStaff.map((staff) => {
      const perf = staffPerformance.find((p) => p.staff_id === staff.id);
      return {
        id: staff.id!.toString(),
        name: staff.name,
        phone: staff.phone,
        email: staff.email || `${staff.name.toLowerCase()}@example.com`,
        position: (staff.position || 'junior') as 'owner' | 'manager' | 'senior' | 'junior' | 'intern',
        role: staff.position,
        hireDate: staff.hire_date || new Date().toISOString(),
        status: 'active' as 'active' | 'on_leave' | 'terminated', // DB에는 status 필드가 없으므로 기본값
        monthlyCustomers: perf?.total_reservations || 0,
        monthlyServices: perf?.completed_reservations || 0,
        salary: staff.salary,
        notes: staff.notes,
      };
    });
  }, [dbStaff, staffPerformance]);
  ...
  const handleStatusToggle = async (staffId: string) => {
    try {
      ...
      // DB에는 status 필드가 없으므로 다른 방식으로 처리하거나 스킵
      await refetchStaff();
      success('직원 상태가 변경되었습니다.');
    } catch (err) {
      error('직원 상태 변경에 실패했습니다.');
    }
    setShowMenuId(null);
  };
  ...
  const handleDeleteStaff = async (staffId: string) => {
    if (confirm('정말로 이 직원을 삭제하시겠습니까?')) {
      try {
        ...
        error(err.message || '직원 삭제에 실패했습니다.');
      }
    }
    setShowMenuId(null);
  };
  ...
  const handleAddStaff = async (newStaff: any) => {
    try {
      ...
      success('새 직원이 추가되었습니다.');
    } catch (err: any) {
      error(err.message || '직원 추가에 실패했습니다.');
    }
  };

  const handleUpdateStaff = async (updatedStaff: any) => {
    try {
      await updateStaff(Number(updatedStaff.id), {
      ...
      error(err.message || '직원 수정에 실패했습니다.');
    }
  };
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      ...
                      <th className="px-4 py-3 text-left font-semibold text-gray-900">직급</th>
                      <th className="px-4 py-3 text-left font-semibold text-gray-900">연락처</th>
                      <th className="px-4 py-3 text-left font-semibold text-gray-900">입사일</th>
                      <th className="px-4 py-3 text-left font-semibold text-gray-900">상태</th>
                      <th className="px-4 py-3 text-left font-semibold text-gray-900">실적</th>
                      <th className="px-4 py-3 text-left font-semibold text-gray-900">액션</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredAndSortedStaff.map((staff) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-block rounded-full px-2 py-1 text-xs font-medium ${getPositionBadgeColor(staff.position)}`}>
                            {getPositionLabel(staff.position)}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="text-sm">
                            <div>
                              <button onClick={(e) => handlePhoneCall(staff.phone, e)} className="text-blue-600 hover:underline">
                                {staff.phone}
                              </button>
                            </div>
                            <div className="text-gray-500">
                              <button onClick={(e) => handleEmailClick(staff.email, e)} className="hover:underline">
                                {staff.email}
                              </button>
                            </div>
                          </div>
                        ...
                        <td className="px-4 py-3">
                          <span className={`rounded-full px-2 py-1 text-xs font-medium ${
                            ...
                            {staff.status === 'active' ? '재직중' : staff.status === 'on_leave' ? '휴직' : '퇴사'}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="text-sm">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            <button onClick={(e) => { e.stopPropagation(); handleEditStaff(staff); }} className="p-1 text-gray-400 transition-colors hover:text-green-600" title="수정">
                              <Pencil size={20} className="text-gray-600 hover:text-blue-600" />
                            </button>
                            <button onClick={(e) => handleViewSchedule(staff, e)} className="p-1 text-gray-400 transition-colors hover:text-purple-600" title="스케줄">
                              <Calendar size={18} className="text-gray-600" />
                            </button>
                          </div>
                        </td>
