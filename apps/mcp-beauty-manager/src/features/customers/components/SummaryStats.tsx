...
export default function SummaryStats({ customer }: SummaryStatsProps) {
  ...
  const totalAmount = customer.visitHistory.reduce(
    (sum, visit) => sum + (visit.amount || 0),
    0
  );
  const averageAmount =
    totalVisits > 0 ? Math.round(totalAmount / totalVisits) : 0;
  ...
  return (
    <div className="p-8">
      <h3 className="mb-6 text-lg font-semibold text-gray-800">고객 통계</h3>

      <div className="grid grid-cols-1 gap-6 md:grid-cols-3 lg:grid-cols-5">
        {stats.map((stat, index) => (
          <Card key={index} hover className="text-center">
            <div
              className={`mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full ${getColorClasses(stat.color)}`}
            >
      ...
      {totalVisits > 0 && (
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2">
          <Card>
            <h4 className="mb-3 font-semibold text-gray-800">방문 패턴</h4>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {totalVisits > 1
                    ? `약 ${Math.round(daysSinceLastVisit / totalVisits)}일`
                    : '신규 고객'}
                </span>
              </div>
            </div>
          ...
          <Card>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {Object.keys(serviceCount).length}개
                </span>
              </div>
              <div className="flex justify-between">
